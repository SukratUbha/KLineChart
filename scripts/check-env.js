...
// Only check package manager during development, not when installing as dependency
if (
  ...
  const currentPackageManager =
    process.env.npm_config_user_agent?.split("/")[0] || "unknown";

  if (currentPackageManager !== allowedPackageManager) {
    console.log(
      styleText(
        "red",
        `\nError: This project must use ${allowedPackageManager} as the package manager. The current package manager used is ${currentPackageManager}. Please use the command ${styleText(
          "underline",
          `${allowedPackageManager} install`
